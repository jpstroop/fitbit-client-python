...
def test_get_bodyfat_timeseries_by_date_range_validates_order(body_timeseries):
...
def test_get_bodyfat_timeseries_by_date_range_max_days(body_timeseries):
    """Test maximum date range limit for body fat."""
    with raises(InvalidDateRangeException) as exc_info:
        body_timeseries.get_bodyfat_timeseries_by_date_range(
            start_date="2023-01-01", end_date="2023-02-01"  # 31 days, exceeds 30 day limit
        )
...
def test_get_bodyfat_timeseries_by_date_range_makes_correct_request(body_timeseries, mock_response):
    ...
    body_timeseries.oauth.request.assert_called_once_with(
        "GET",
        "https://api.fitbit.com/1/user/test_user/body/log/fat/date/2023-01-01/2023-01-10.json",
        params=None,
        data=None,
        json=None,
