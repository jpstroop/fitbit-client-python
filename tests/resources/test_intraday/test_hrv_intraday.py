# tests/resources/test_intraday/test_hrv_intraday.py
...
def test_get_hrv_intraday_by_date_success(
    ...
    """Test successful retrieval of intraday HRV data"""
    expected_response = {
        "hrv": [
            {
                "dateTime": "2024-02-13",
...
def test_get_hrv_intraday_interval_all_endpoint(
    ...
    """Test HRV interval constructs 'all' endpoint (lines 587-588)"""
    mock_response = mock_response_factory(200, {"hrv": []})
    mock_oauth_session.request.return_value = mock_response

    result = intraday_resource.get_hrv_intraday_by_interval(
...
def test_get_hrv_intraday_by_date_all_endpoint(
    ...
    """Test HRV intraday endpoint with 'all' parameter"""
    mock_response = mock_response_factory(200, {"hrv": []})
    mock_oauth_session.request.return_value = mock_response

    result = intraday_resource.get_hrv_intraday_by_date("2024-02-13")
...
def test_get_hrv_intraday_by_date_range_validation(intraday_resource):
    """Test date range validation for HRV intraday"""
...
def test_get_hrv_intraday_by_interval_invalid_dates(intraday_resource):
    """Test invalid dates for HRV interval"""
