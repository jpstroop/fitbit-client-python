# tests/fitbit_client/utils/test_pagination_validation.py
...
def test_validate_pagination_params_success():
...
def test_validate_pagination_params_missing_dates():
...
def test_validate_pagination_params_sort_validation():
...
def test_validate_pagination_params_custom_field_names():
...
def test_validate_pagination_params_non_enum_sort():
    """Test validation catches non-enum sort values"""
    ...
    with raises(PaginationException) as exc_info:
        dummy_func(before_date="2024-02-13", sort="desc")
    assert "Sort must be a SortDirection enum value" in str(exc_info.value)
    assert exc_info.value.field_name == "sort"

    # Test with integer value instead of enum
    with raises(PaginationException) as exc_info:
        dummy_func(before_date="2024-02-13", sort=1)
    assert "Sort must be a SortDirection enum value" in str(exc_info.value)
    assert exc_info.value.field_name == "sort"
